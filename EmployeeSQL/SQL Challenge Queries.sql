...
--solution 1  
SELECT 
    (SELECT DMJ.dept_no FROM Department_Manager_Junction DMJ WHERE DMJ.Emp_No = E.Emp_No) AS Dept_No,
    (SELECT D.dept_name FROM Departments D WHERE D.Dept_No = (SELECT DMJ.dept_no FROM Department_Manager_Junction DMJ WHERE DMJ.Emp_No = E.Emp_No)) AS Dept_Name,
    E.Emp_No,
    E.Last_Name,
    E.First_Name
FROM Employees E
WHERE E.Emp_No IN (SELECT DMJ.Emp_No FROM Department_Manager_Junction DMJ);
...
FROM employees
GROUP BY last_name
ORDER BY frequency_of_names DESC, last_name ASC;
